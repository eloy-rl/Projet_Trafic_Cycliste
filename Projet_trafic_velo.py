...
def axes_analyses():
    st.header("2. Axes d'analyses")
    axes = [
        "a. Coût de stationnement sur voies publiques des véhicules motorisés et trafic vélo",
        "b. Interdiction de stationnement motorisé sur la voie publique et trafic cycliste",
        "c. Caractéristiques statistiques INSEE des arrondissements et trafic cycliste",
...
Cela signifie que le positionneCela s’explique tout simplement car population et surface ne sont pas les critères d’installation et dimensionnement des stations Vélib.
...
"""
    ...
    for axe in axes:
        with st.expander(axe, expanded=False):
            ...
            if axe in analyses:
                st.write(analyses[axe])
            else:
                st.write("Analyse non disponible.")
